...
function DeletePlanDialog({ plan, onDeleted }: { plan: DoctorPlan; onDeleted: () => void }) {
  ...
  if (plan.isDefault) {
    return (
      <Button
        className="size-8 cursor-not-allowed text-muted-foreground"
        disabled
        size="icon"
        title="Default plan cannot be deleted"
        variant="ghost"
      >
...
function DoctorPlansRoute() {
  const { data: stats } = useSuspenseQuery(
    orpc.planStats.queryOptions()
  );
  const { data: plansData } = useSuspenseQuery(

...
function HubMaterialDetailPage() {
  ...
  function getMaterialFileUrl(fileKey: string | null | undefined): string | null {
    if (!fileKey) return null;
    const { VITE_SERVER_URL } = (import.meta as any).env;
    return `${VITE_SERVER_URL}/media/${encodeURIComponent(fileKey)}`;
  }

  const mediaUrl = getMaterialFileUrl(material.fileKey);
          ...
          <div className="relative flex aspect-video items-center justify-center overflow-hidden rounded-[2rem] border border-border/40 bg-black/5">
            {material.status === "ready" && mediaUrl && isVideo ? (
              <video
                ...
                controls
                playsInline
                poster={material.thumbnailKey ? getMaterialFileUrl(material.thumbnailKey) ?? undefined : undefined}
                preload="metadata"
                src={mediaUrl}
              />

...
import { CATEGORIES, hospitals, type Hospital } from "@/data/hospitals";
...
export default function MapScreen() {
  ...
  const openMapsNavigation = useCallback((hospital: Hospital) => {
  ...
  const detailHospital = showDetailModal ? (selectedHospital ?? hospitals[0]) : null
  ...
  return (
    <>
      ...
      <View className="flex-1 bg-background">
        ...
        <View className="absolute top-0 left-0 right-0 px-4 pt-14">
          <View className="relative">
            <View className="flex-row items-center gap-2 rounded-xl border-2 border-border bg-card px-3 py-2.5 shadow-lg">
              <Search color={colors.mutedForeground} size={16} strokeWidth={2.5} />
              <TextInput
                ref={searchInputRef}
                value={search}
                onChangeText={(text) => {
                    ...
                    filteredDoctors.map(({ profile, affiliations }) => (
                      ...
                      >
                        <View className="mt-1 h-9 w-9 items-center justify-center rounded-full bg-secondary">
                          <Stethoscope color={colors.mutedForeground} size={16} strokeWidth={2} />
                        </View>
                        <View className="flex-1 gap-1">
                          ...
                          {profile.headline && (
                            <Text className="font-medium font-sans text-muted-foreground text-xs" numberOfLines={1}>
                              {profile.headline}
                            </Text>
                          )}
                          {affiliations.length > 0 && (
                            <View className="flex-row flex-wrap gap-1">
                              {affiliations.map((aff) => (
                                <Tag key={aff.tenantId} size="sm" variant="muted">
                                  {aff.tenantName}
                                </Tag>
                              ))}
                            </View>
        ...
        <Animated.View
          className="absolute top-0 bottom-0 right-0 z-20 border-l-2 border-border bg-card shadow-xl"
          style={{
            width: panelWidth,
        ...
        >
          <View className="flex-row items-center justify-between border-border border-b px-4 py-3">
            ...
            <Pressable onPress={togglePanel} className="h-8 w-8 items-center justify-center rounded-full bg-secondary">
              ...
              return (
                ...
                >
                  <View className="h-9 w-9 items-center justify-center rounded-full bg-primary/15">
                    <HospitalIcon color={colors.primary} size={16} strokeWidth={2} />
                  </View>
                  <View className="flex-1 gap-0.5">
                    <Text className="font-bold font-sans text-foreground text-xs uppercase tracking-tight" numberOfLines={1}>
                      {hospital.name}
                    </Text>
                    <View className="flex-row items-center gap-1.5">
                      <Star color={colors.warning} size={10} strokeWidth={2} fill={colors.warning} />
                      <Text className="font-medium font-sans text-muted-foreground text-[10px]">{hospital.rating}</Text>
            ...
            <View className="gap-3 rounded-xl border-2 border-border bg-card p-4 shadow-lg">
              <View className="flex-row items-start justify-between">
                <View className="flex-1 gap-1 pr-4">
                  <Text className="font-black font-sans text-foreground text-base leading-tight uppercase tracking-tight">
                    {selectedHospital.name}
                  </Text>
                  <View className="flex-row items-center gap-2">
                    <View className="flex-row items-center gap-1">
                      <Star color={colors.warning} size={12} strokeWidth={2.5} fill={colors.warning} />
                      <Text className="font-bold font-sans text-foreground text-xs">{selectedHospital.rating}</Text>
                    </View>
                  ...
                  <View className="mt-1 flex-row items-start gap-1.5">
                    <MapPin color={colors.mutedForeground} size={12} strokeWidth={2} />
                    <Text className="flex-1 font-medium font-sans text-muted-foreground text-xs leading-snug" numberOfLines={2}>
              ...
              <View className="flex-row gap-2">
                {selectedHospital.phone && (
                  ...
                  >
                    <View className="flex-row items-center gap-1.5">
                      ...
                      <Text className="font-bold font-sans text-primary-foreground text-[10px] uppercase">Call</Text>
                    </View>
                  </Button>
                )}
                {selectedHospital.website && (
                  ...
                  >
                    <View className="flex-row items-center gap-1.5">
                      <Globe color={colors.foreground} size={14} strokeWidth={2.5} />
                      <Text className="font-bold font-sans text-foreground text-[10px] uppercase">Website</Text>
                    </View>
                  </Button>
                )}
                <Button variant="outline" size="sm" className="flex-1" onPress={() => openMapsNavigation(selectedHospital)}>
                  <View className="flex-row items-center gap-1.5">
                    <Navigation color={colors.foreground} size={14} strokeWidth={2.5} />
                    <Text className="font-bold font-sans text-foreground text-[10px] uppercase">Navigate</Text>
                  </View>
                </Button>
              </View>
              ...
              >
                <Info color={colors.mutedForeground} size={14} strokeWidth={2} />
                <Text className="font-bold font-sans text-muted-foreground text-[10px] uppercase tracking-wider">
                  View details & clinics
                </Text>
                <ChevronRight color={colors.mutedForeground} size={14} strokeWidth={2.5} />
              </Pressable>
            </View>
          </View>
        )}
        ...
        <View className="flex-1 bg-black/50">
          <View className="mt-auto max-h-[85%] rounded-t-3xl border-2 border-border bg-card">
            <View className="flex-row items-center justify-between border-border border-b px-5 py-4">
              <Text className="flex-1 font-black font-sans text-foreground text-lg uppercase tracking-tight" numberOfLines={1}>
                {detailHospital?.name}
              </Text>
              <Pressable
            ...
            <ScrollView className="px-5 py-4" contentContainerClassName="gap-5 pb-8">
              <View className="gap-3">
                <View className="flex-row items-center gap-2">
                  <Star color={colors.warning} size={14} strokeWidth={2.5} fill={colors.warning} />
                ...
                {detailHospital?.phone && (
                  <Pressable
                    ...
                    onPress={() => Linking.openURL(`tel:${detailHospital!.phone}`)}
                  >
                    <Phone color={colors.primary} size={14} strokeWidth={2} />
                    <Text className="font-medium font-sans text-primary text-sm">{detailHospital?.phone}</Text>
                  </Pressable>
                )}
                ...
                {detailHospital?.website && (
                  ...
                  >
                    ...
                    <Text className="flex-1 font-medium font-sans text-primary text-sm" numberOfLines={1}>
                      {detailHospital?.website}
                    </Text>
                  </Pressable>
                )}
              ...
              <View className="gap-3">
                <View className="flex-row items-center gap-2">
                  <Clock color={colors.mutedForeground} size={14} strokeWidth={2} />
                ...
                <View className="rounded-xl border-2 border-border bg-background p-3">
                  {detailHospital?.hours ? (
                    DAYS.map((day) => {
                      const hours = detailHospital.hours?.[day as keyof typeof detailHospital.hours]
                      return (
                        <View key={day} className="flex-row items-center justify-between py-1.5">
                          <Text className="font-bold font-sans text-foreground text-xs">{day}</Text>
                          <Text className="font-medium font-sans text-muted-foreground text-xs">
                            {hours ?? "Not available"}
                          </Text>
                        </View>
                    ...
                    <View className="py-2">
                      <Text className="font-medium font-sans text-muted-foreground text-xs text-center">
              ...
              <View className="gap-3">
                <View className="flex-row items-center gap-2">
                  <HospitalIcon color={colors.mutedForeground} size={14} strokeWidth={2} />
                  <Text className="font-bold font-sans text-foreground text-xs uppercase tracking-wider">
                    Clinics & Specializations
                  </Text>
                </View>
                <View className="rounded-xl border-2 border-border bg-background p-4">
                  <Text className="font-medium font-sans text-muted-foreground text-xs text-center leading-relaxed">
                    Clinic information will be available once the hospital is registered
                    on our platform.
                  </Text>
                </View>
              </View>
              ...
              <View className="flex-row gap-3">
                ...
                <Button variant="outline" size="sm" className="flex-1" onPress={() => openMapsNavigation(detailHospital!)}>
                  <View className="flex-row items-center gap-1.5">
                    <Navigation color={colors.foreground} size={14} strokeWidth={2.5} />
                    <Text className="font-bold font-sans text-foreground text-[10px] uppercase">Navigate</Text>
                  </View>
      ...
      <ScreenBottomBar
        actions={FILTER_ACTIONS.map((a) => ({ icon: a.icon, label: a.label, value: a.value }))}
      ...
      >
            ...
            return (
              ...
              >
                {({ pressed }) => (
                  ...
                  >
                    <Icon color={isActive ? "#ffffff" : colors.mutedForeground} size={20} strokeWidth={2.5} />
                  </View>
                )}
              </Pressable>
            )
        ...
        {selectedHospital && (
          <View className="flex-row gap-2">
            {selectedHospital.phone && (
              <IconButton icon={Phone} iconSize={16} onPress={() => Linking.openURL(`tel:${selectedHospital.phone}`)} />
            )}
            <IconButton icon={Navigation} iconSize={16} onPress={() => openMapsNavigation(selectedHospital)} />
          </View>
        )}

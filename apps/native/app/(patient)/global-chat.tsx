...
export default function GlobalChatScreen() {
  ...
  const sendMutation = useMutation(
    orpc.chatRoom.send.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: orpc.chatRoom.key() });
      },
      onError: (error) => {
        const message =
  ...
  const allMessages = [
    ...initialMessages.filter(
      (m) => !liveMessages.some((lm) => lm.id === m.id)
    ),
    ...liveMessages,
  ];
  ...
  return (
    <View className="flex-1 items-center bg-background">
      <View className="w-full max-w-xl flex-1 bg-background">
        <Stack.Screen
          options={{
            headerShown: false,
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View className="flex-1 items-center justify-center gap-lg px-xl">
                ...
                <Text className="font-sans text-caption text-foreground-muted text-center">

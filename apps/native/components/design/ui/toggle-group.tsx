...
interface ToggleGroupItem<T> {
  label: string;
  value: T;
  textClassName?: string; // per-item text styling
}

interface ToggleGroupProps<T> {
  className?: string;
  textClassName?: string; // default text styling for all items
...
export function ToggleGroup<T extends string | number>({
  ...
  return (
    <View
      className={`relative ${isMultiRow? "flex-row flex-wrap" : "flex-row"} rounded-2xl border-border/70 bg-background-subtle p-1 ${className?? ""}`.trim()}
